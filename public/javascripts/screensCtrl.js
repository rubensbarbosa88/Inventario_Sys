app.controller('screensCtrl', function ($scope, $http) {
	$scope.itens = [];
	$scope.arrayCsv = [];
	$scope.itemObj = {};
	$scope.procurar = "";
...
//getDb function
	function refresh(){
		$http.get('/monitores/inventscreens').success(function(data){
				$scope.itens = data;
					...
					$scope.arrayCsv.push(
						{
							...
							sn:data[o].sn,
							user:data[o].user,
							departamento:data[o].departamento,
							ref:data[o].ref,
...
//-Order list function
	...
	$scope.modelo = function(){
		if($scope.orderValidate[3] == 0){
			$scope.order = '-modelo';
			$scope.orderShow = "Modelo";
			$scope.orderValidate[3] += 1;
		}
		else if($scope.orderValidate[3] == 1){
			$scope.order = 'modelo';
			$scope.orderShow = "Modelo";
			$scope.orderValidate[3] -= 1;
		}		
	...
	$scope.user = function(){
		if($scope.orderValidate[5] == 0){
			$scope.order = '-user';
			$scope.orderShow = "Usuário";
			$scope.orderValidate[5] += 1;
		}
		else if($scope.orderValidate[5] == 1){
			$scope.order = 'user';
			$scope.orderShow = "Usuário";
			$scope.orderValidate[5] -= 1;
		}
	}
	$scope.departamento = function(){
		if($scope.orderValidate[6] == 0){
			$scope.order = '-departamento';
			$scope.orderShow = "Departamento";
	...
	$scope.enviar = function(form){
		$scope.submitted = true;
			if(form.$invalid){
				...
				$http.post('/monitores/inventscreens', $scope.itemObj).success(function(data){
					$scope.submitted = false;
					$scope.itemObj = "";
					angular.element(
...
//UPDATE A ITEM
	$scope.update = function(id, item){
		$http.put('/monitores/inventscreens/' + id, item).success(function(data){
			angular.element(
...
//DELETE ITEM
	$scope.delete = function(id, item){
		...
		$scope.sim = function(){
			//Delete	
			$http.delete('/monitores/inventscreens/' + id, item).success(function(data){
