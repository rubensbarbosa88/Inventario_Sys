...
var db = mongojs('mongodb://reamp:reamp#159@ds017544.mlab.com:17544/inventario_db', ['moveis']);


var inventarioController = {
	getDb: function(req, res){
			db.moveis.find(function(err, list){
				res.json(list);
			});
		},
	postDb: function(req, res){
			db.moveis.insert(req.body, function(err, list){
				res.json(list);
			});
	},
	updDb: function(req, res){
			var id = req.params.id;
				...
				db.moveis.findAndModify({query: {_id: mongojs.ObjectId(id)},
	...
	delDb: function(req , res){
			...
			var id = req.params.id;
			console.log(id);
			db.moveis.remove({_id: mongojs.ObjectId(id)}, function(err, docs){
				if(err){
					return;
					console.log(err);
				}
